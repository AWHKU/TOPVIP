...
def next_sample(Prob,Fitness,AACombo,Fit,SEQ,Fit_list,SEQ_list,SEQ_index,Index):
    for cluster_id in range(len(Index)):
        if len(Index[cluster_id]) == 0:
            Prob[cluster_id] = 0
    if len([p for p in Prob if p ==0]) > 0:
        Prob=[1]* len(Prob)
    Prob = linear_prob(Prob)
    # Prob = Prob/np.sum(Prob)
...
def cluster_sample(args,save_dir,features,features_zeroshot,AACombo, Fitness,ComboToIndex, groundtruth):
    ...
    while num < num_training_data:
        ...
        num += 1
    ### return original zeroshot values ###
    Fit_list = groundtruth[groundtruth["Variants"].isin(SEQ_list)]
    Fit_list=Fit_list["Fitness"].values
    ...
    #### random select 48 samples 
    ##sub_data = sub_data.sample(n=48, random_state=1)
    v_name=args.prefix + '_InputValidationData.csv'
    trainingdata=os.path.join(save_dir , v_name)
    sub_data.to_csv(trainingdata, index=False)
    cluster_file=args.prefix + 'clustering.npz'
    np.savez(os.path.join(save_dir, cluster_file), tree=tree)
    return trainingdata
...
def main_sampling(seed,args,save_dir):
    ...
    # get feature matrix
    encoding_lib = args.zeroshot_features
    features_zeroshot = np.load(encoding_lib)
    if len(features_zeroshot.shape) ==1:
        features_zeroshot=features_zeroshot.reshape(-1,1)
        ### force it to be a 2-d array for later analyses
        features_zeroshot=np.column_stack((features_zeroshot, features_zeroshot))
    encoding_lib = args.encoding 
    features = np.load(encoding_lib)
    Comb_ind_file=args.comb_ind
    ComboToIndex=pickle.load(open(Comb_ind_file,'rb'))
    ...
    trainingdata=cluster_sample(args,save_dir,features,features_zeroshot,AACombo, Fitness,ComboToIndex, groundtruth)
    return trainingdata
...
if __name__ == "__main__":
    ...
    parser.add_argument("--xlsx", help = "excel file with zeroshot/Fitness")
    parser.add_argument("--zeroshot_features", help = "zeroshot_features")
    parser.add_argument("--comb_ind", help = "Combo to Index pkl")
    parser.add_argument("--prefix", help = "prefix name for output data")
