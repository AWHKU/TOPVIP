...
### def get_data
data_filename="GFP_VAE/GFP_homologues_w1.fasta"
...
#weights["0.1"].size()
#torch.Size([999])
torch.save(weights, "GFP_VAE/GFP_homologues_AACombo_weightsz30.pkl")
...
trainer.save_checkpoint("GFP_VAE/GFP_homologues_modelz30.ckpt")
encoded_train=model.embedding(data)
encoded_train=encoded_train.detach().cpu().numpy()
import pandas as pd
encoded_train=pd.DataFrame(encoded_train)
encoded_train["AACombo"]=labels
encoded_train.to_csv("GFP_VAE/GFP_homologues_representationz30.csv")


# embed new features?
### load test data ###
testdata_filename="GFP_VAE/GFP_mutants.fasta"
test_ids = []
test_labels = []
test_seqs = []
...
np.save("GFP_VAE/GFP_variants_AACombo_VAEz30_embedded.npy", test_embedded.cpu().detach().numpy())
...
import pickle
with open('GFP_VAE/GFP_variants_AACombo_VAEz30_IndexToCombo.pkl', 'wb') as handle:
    pickle.dump(ind_to_combo, handle, protocol=pickle.HIGHEST_PROTOCOL)

with open('GFP_VAE/GFP_variants_AACombo_VAEz30_ComboToIndex.pkl', 'wb') as handle:
