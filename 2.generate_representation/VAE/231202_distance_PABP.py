...
import torchplot
import sys
#sys.path.append("/data/users/athchu/meaning_rep/meaningful-protein-representations/models")
from vae_geometric import VAE, get_hparams, aa1_to_index, aa1, numeric_curve_optimizer


### def get_data
data_filename="data/zeroshot_prediction/EVcoupling/PABP_homologues_w1.fasta"
...
#weights["0.1"].size()
#torch.Size([999])
torch.save(weights, "PABP_VAE/PABP_homologues_AACombo_weightsz30.pkl")
...
trainer.save_checkpoint("PABP_VAE/PABP_homologues_modelz30.ckpt")
encoded_train=model.embedding(data)
encoded_train=encoded_train.detach().cpu().numpy()
import pandas as pd
encoded_train=pd.DataFrame(encoded_train)
encoded_train["AACombo"]=labels
encoded_train.to_csv("PABP_VAE/PABP_homologues_representationz30.csv")


# embed new features?
### load test data ###
testdata_filename="PABP_VAE/PABP_mutants.fasta"
test_ids = []
test_labels = []
test_seqs = []
...
np.save("PABP_VAE/PABP_variants_AACombo_VAEz30_embedded.npy", test_embedded.cpu().detach().numpy())
...
import pickle
with open('PABP_VAE/PABP_variants_AACombo_VAEz30_IndexToCombo.pkl', 'wb') as handle:
    pickle.dump(ind_to_combo, handle, protocol=pickle.HIGHEST_PROTOCOL)

with open('PABP_VAE/PABP_variants_AACombo_VAEz30_ComboToIndex.pkl', 'wb') as handle:
