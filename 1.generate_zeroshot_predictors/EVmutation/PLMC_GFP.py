#### cd /home/athchu/data/2022_09_16_EVcoupling
...
import pandas as pd
import sys
sys.path.append('/combining-evolutionary-and-assay-labelled-data/src')
...
m_path="data/zeroshot_prediction/EVcoupling_alignments/GFP/GFP_AEQVI.model_paramsGFP_AEQVI.model_params"
couplings_model = CouplingsModel(m_path)
### begin couplings_model.py 
precision="float32"
file_format="plmc_v2"
wtseqs, wtids = read_fasta("data/zeroshot_prediction/EVcoupling/GFP_wt.fasta", return_ids=True)
...
data=pd.read_csv("data/zeroshot_prediction/arDCA")
seq2mutation(data.seq.values[0], couplings_model, offset=couplings_model.offset, ignore_gaps=False)


X=seq2effect(data.seq.values, couplings_model, couplings_model.offset, ignore_gaps=False)
data['plmc']=X 

data.to_csv("GFP_fitness_EV.csv")
